...
const handleTrain = async (row) => {
  const res = await trainFusionModel([row.key]);
  if (res.code === 200) {
    ElNotification({
      title: "模型训练",
      message: "模型开始训练",
...
const handleTrainAll = async () => {
  const res = await trainFusionModel(data.value.map((item) => item.key));
  if (res.code === 200) {
    ElNotification({
      title: "训练成功",
      message: "模型训练成功",
  ...
  <div class="input_box">
    <el-form :inline="true" :model="form" class="demo-form-inline">
      ...
      <el-form-item>
        ...
        <TrainAllButton :data="data" :set-data="setData" :fusion="true" />
      </el-form-item>
    </el-form>
  </div>
  <!-- 数据展示 -->
